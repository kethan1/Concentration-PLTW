  ...
  public Board() {
    ...
    for (int c = 0; c < gameboard.length; c++) {
      for (int r = 0; r < gameboard[0].length; r++) {
        int random_idx = (int) (Math.random() * cTileValues.size());
        String tileVal = cTileValues.get(random_idx);
        gameboard[c][r] = new Tile(tileVal);
        cTileValues.remove(random_idx);
      }
    }
  }
